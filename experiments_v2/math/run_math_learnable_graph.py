"""
MATH experiment with LearnableMultiNodeGraph.
...
Usage:
    python experiments_v2/math/run_math_learnable_graph.py --batch_size 4 --num_iterations 100

    # With custom topology
    python experiments_v2/math/run_math_learnable_graph.py --mode Chain --num_rounds 1

    # Resume training from checkpoint
    python experiments_v2/math/run_math_learnable_graph.py --resume_from checkpoints/graph_iter50.pt

    # Evaluate with hard selection
    python experiments_v2/math/run_math_learnable_graph.py --eval_only --resume_from checkpoints/graph_best.pt
"""
...
# Default multi-node configurations for MATH (3 specialized math roles)
...
def load_math_dataset(dataset_json: str, max_samples: int = None) -> list:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MATH experiment with LearnableMultiNodeGraph"
    )
    ...
    parser.add_argument(
        '--valid_dataset_json',
        type=str,
        default='Datasets/MATH/math_valid.jsonl',
    ...
    parser.add_argument(
        ...
        type=str,
        default='gsm8k',
        help='Domain for prompts (MATH reuses gsm8k prompts)'
    )
...
async def eval_step(
    ...
    if max_concurrent is not None and max_concurrent > 0:
        ...
        tasks = [limited_process(sample, copy.deepcopy(graph)) for sample in batch]
    else:
        tasks = [process_sample(sample, copy.deepcopy(graph)) for sample in batch]
...
async def main():
    ...
    print("=" * 80)
    print("LearnableMultiNodeGraph Experiment - MATH")
    print("=" * 80)
    ...
    if args.result_file:
        ...
        result_dir = Path(f"{HIERAMAS_ROOT}/result_v2/math/stage1")
    ...
    # Load dataset
    dataset = load_math_dataset(args.dataset_json, args.max_samples)
    print(f"Dataset size: {len(dataset)}")

    # Load validation dataset
    valid_dataset = None
    if args.valid_dataset_json:
        valid_dataset = load_math_dataset(args.valid_dataset_json, args.max_valid_samples)
